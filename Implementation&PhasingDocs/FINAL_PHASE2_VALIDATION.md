...
> Updated 2026-07-16 during the quantitative-enhancements corrective pass
> (`TradingHub_Quantitative_Enhancements_Final_Audit_and_Corrective_Prompt.md`).
> The line below about financing was stale by the time of that audit — financing
> is now implemented (as synthetic/configured rates), just not yet broker-sourced.
> See `VALIDATION.md` for the full implemented/synthetic/uncertified/tested
> breakdown across every feature this project now includes.

- OANDA historical execution remains midpoint plus configured spread, not true historical bid/ask (**implemented but synthetic**);
- native OANDA protective-stop amendment remains capability-dependent; the simulator path is deterministic (**implemented but not certified** against a live/demo broker);
- overnight financing/swap **is now modelled** (**implemented but synthetic** — user/configuration-supplied long/short annual rates, not sourced from a broker feed);
- price-action thresholds are calibrated from warm-up, but outcome-sensitive strategy parameters are not automatically optimised (a QuantResearch parameter-sensitivity/walk-forward runner exists as library code but has no operational CLI workflow yet — **not implemented** as an end-to-end tool);
