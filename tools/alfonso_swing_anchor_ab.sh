#!/bin/bash
# 3.54 A/B: swing anchored at the base end (A) vs at the extreme (B).
# Reproduces the PROJECT_STATE.md 3.54 A/B. Output root is overridable: ALFONSO_AB_OUT=... ./tools/alfonso_swing_anchor_ab.sh
...
cd "$(dirname "$0")/.." || exit 1
OUT=${ALFONSO_AB_OUT:-/mnt/storage/scratch/alfonso/m3ab}
...
for tag in "${!INST[@]}"; do
  dotnet run --project BacktestRunner -c Release --no-build -- \
    ...
    --alfonso-swing-anchor-at-base-end \
    --alfonso-candidate-log $OUT/a-$tag.csv --output $OUT/a-$tag \
    > $OUT/a-$tag.log 2>&1 &
  sleep 1
done
wait
echo "ARM A (anchor at base end) COMPLETE"
...
wait
echo "ARM B (anchor at extreme) COMPLETE"
