#!/bin/bash
# 3.65 A/B: contradicting trendlines allowed (A) vs rejected (B).
# Reproduces the PROJECT_STATE.md 3.65 A/B. Output root is overridable: ALFONSO_AB_OUT=... ./tools/alfonso_trendline_slope_ab.sh
...
cd "$(dirname "$0")/.." || exit 1
OUT=${ALFONSO_AB_OUT:-/mnt/storage/scratch/alfonso/m6ab}
...
for tag in "${!INST[@]}"; do
  dotnet run --project BacktestRunner -c Release --no-build -- \
    ...
    --alfonso-allow-contradicting-trendlines \
    --alfonso-candidate-log $OUT/a-$tag.csv --output $OUT/a-$tag \
    > $OUT/a-$tag.log 2>&1 &
  sleep 1
done
wait
echo "ARM A (contradicting lines allowed, the old fit) COMPLETE"
...
wait
echo "ARM B (contradicting lines rejected, the new default) COMPLETE"
