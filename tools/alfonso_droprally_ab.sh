#!/bin/bash
# 3.53 A/B: drop/rally base = one candle (A) vs both ERCs (B).
# Reproduces the PROJECT_STATE.md 3.53 A/B. Output root is overridable: ALFONSO_AB_OUT=... ./tools/alfonso_droprally_ab.sh
...
cd "$(dirname "$0")/.." || exit 1
OUT=${ALFONSO_AB_OUT:-/mnt/storage/scratch/alfonso/m2ab}
...
for tag in "${!INST[@]}"; do
  dotnet run --project BacktestRunner -c Release --no-build -- \
    ...
    --alfonso-single-candle-drop-rally \
    --alfonso-candidate-log $OUT/a-$tag.csv --output $OUT/a-$tag \
    > $OUT/a-$tag.log 2>&1 &
  sleep 1
done
wait
echo "ARM A (single candle) COMPLETE"
