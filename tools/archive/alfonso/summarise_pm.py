...
for t in T:
    ...
    cells={}
    for tag,d in (('off',f'p-{t}-off'),('on',f'p-{t}-on')):
        try:
...
pos_off=sum(1 for t in T if os.path.exists(f'p-{t}-off/simulation-result.json') and perf(f'p-{t}-off')['averageR']>0)
pos_on =sum(1 for t in T if os.path.exists(f'p-{t}-on/simulation-result.json')  and perf(f'p-{t}-on')['averageR']>0)
