...
/// <summary>
/// Compatibility identity provider for <c>SupplyDemandPullbackPlaybook</c>, mirroring
...
/// of <c>SupplyDemandPullbackPlaybook.Version</c> - bump it by hand whenever the playbook's own
/// decision logic changes in a calibration-relevant way.
/// </summary>
public sealed class SupplyDemandPullbackCalibrationCompatibility :
    ICalibrationCompatibilityProvider<SupplyDemandPullbackOptions>
{
    public static SupplyDemandPullbackCalibrationCompatibility Instance { get; } = new();

    /// <summary>Matches <c>SupplyDemandPullbackPlaybook.StableId</c> so artifacts and playbook identity never drift apart.</summary>
    public string StrategyId => "structural.supply-demand-pullback";
    public string StrategyImplementationVersion => "1.0";
    public string OptionsSchemaVersion => "supply-demand-pullback-options-v1";

    public string ComputeDefaultConfigurationHash() =>
        IndicatorCalibrationHash.ComputeOfObject(new SupplyDemandPullbackOptions());
